...
export class DefinitionValidationService {
    validateDefinition(definition: RawDefinition) {
        ...
        throw new CommandError(
            `Invalid definition format in file '${definition.fileName}'\n${error.message}`,
        );
    }
    ...
    private validateDuplicatedKS(definition: Definition) {
        ...
        definition.knowledgeSources.forEach((ks) => {
            if (slugs.has(ks.slug)) {
                throw new CommandError(
                    `Duplicated knowledge source slug '${ks.slug}' found on ${ks.fileName}`,
                );
            }
    ...
    private validateDuplicatedKO(ks: KnowledgeSource) {
        ...
        ks.knowledgeObjects.forEach((ko) => {
            if (slugs.has(ko.slug)) {
                throw new CommandError(
                    `Duplicated knowledge object slug '${ko.slug}' in source '${ks.slug}' found on ${ks.fileName}`,
                );
            }
    ...
    private validateSlugs(ks: KnowledgeSource) {
        ...
        if (invalidSlugs.length) {
            throw new CommandError(
                `Invalid knowledge object slug(s) found in knowledge source '${ks.slug}' on file '${ks.fileName}:'\n${invalidSlugs.join('\n')}`,
            );
        }
        ...
        if (ksSlugError) {
            throw new CommandError(
                `Invalid knowledge source slug '${ks.slug}' found on file '${ks.fileName}':\n${ksSlugError}`,
            );
        }
    }
    ...
    private validateEmptyContent(ks: KnowledgeSource) {
        ks.knowledgeObjects.forEach((ko) => {
            if (!ko.content) {
                throw new CommandError(
                    `Knowledge object '${ko.slug}' in source '${ks.slug}' on file '${ks.fileName}' has no content`,
                );
            }
        });
    }
