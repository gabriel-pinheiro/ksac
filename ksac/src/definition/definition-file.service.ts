...
export class DefinitionFileService {
    ...
    async scanFiles(): Promise<string[]> {
        ...
        if (!files.length) {
            throw new CommandError(
                'No definition files found. Make sure to create your definitions with the .hcl extension',
            );
        }
    ...
    private async parseFile(fileName: string): Promise<DefinitionFile> {
        ...
        const content = await readFile(fileName).catch(Hoek.ignore);
        if (!content) {
        ...
        if (error) {
            throw new CommandError(
                `Failed to parse file '${fileName}:${error.Pos.Line}:${error.Pos.Column}'`,
            );
        }
    ...
    private async getDefinitionFiles(path: string): Promise<string[]> {
        ...
        const fileNames = await readdir(path).catch(Hoek.ignore);

        if (!fileNames) {
            throw new CommandError(
                `Failed to load definitions from '${path}/', make sure it's a directory and your user has read permission on its files`,
            );
        }
        ...
        const subDirContents = await this.getSubDirectoryContents(
            path,
            subDirs,
        );

        const dirContents = definitionFiles.map((d) => join(path, d.name));
    ...
    private async getSubDirectoryContents(
        path: string,
        subDirs: FileOrDir[],
    ): Promise<string[]> {
        const subDirContents: string[] = [];
        for (const d of subDirs) {
            const dirContents = await this.getDefinitionFiles(
                join(path, d.name),
            );
