...
import {
    FIELDS,
    PartialKnowledgeSource,
    UpdateKSStep,
} from './steps/update-ks-step';
...
export class PlanService {
    ...
    private async getKnowledgeSourceSteps(
        ...
        if (!ksSteps.length && !koSteps.length) {
            debug(
                `knowledge source '${desired.slug}' is up to date, no steps added`,
            );
            return [];
        }
        ...
        if (!ksSteps.length && koSteps.length) {
            debug(
                `knowledge source '${desired.slug}' is up to date, but kos must be updated, adding 'keep' step`,
            );
    ...
    private async getKSOnlySteps(desired: KnowledgeSource): Promise<Step[]> {
        ...
        console.log(
            `Creating plan for Knowledge Source '${desired.slug.bold}'`,
        );
        const steps = [];
        ...
        if (!current) {
            debug(
                `knowledge source '${desired.slug}' not found, adding create step`,
            );
        ...
        if (mustUpdate) {
            debug(
                `knowledge source '${desired.slug}' must be updated, adding update step`,
            );
    ...
    private async getKOSteps(ks: KnowledgeSource): Promise<Step[]> {
        ...
        for (const current of currentKOs) {
            const desired = desiredKOs.find((ko) =>
                this.compareKOs(current, ko),
            );
            if (desired) {
                continue;
            }
            ...
            debug(
                `knowledge object '${current.metadata.custom_id}' not defined, adding delete step`,
            );
            steps.push(DeleteKOStep.of(ks.slug, current));
        }

        for (const desired of desiredKOs) {
            const current = currentKOs.find((ko) =>
                this.compareKOs(ko, desired),
            );
            if (current) {
            ...
            debug(
                `knowledge object '${desired.slug}' not found, adding create step`,
            );
    ...
    private compareKOs(
        current: ApiKnowledgeObject,
        desired: KnowledgeObject,
    ): boolean {
        return (
            current.page_content === desired.content &&
            current.metadata.language === desired.language &&
            current.metadata.hint === desired.useCases
        );
    }

    private async fetchKnowledgeSource(
        slug: string,
    ): Promise<PartialKnowledgeSource> {
    ...
    private async fetchKnowledgeObjects(
        ksSlug: string,
    ): Promise<ApiKnowledgeObject[]> {
        const stk = await this.authService.getStackSpot();

        try {
            debug(
                `fetching knowledge objects for knowledge source '${ksSlug}'`,
            );
    ...
    private getDestroyPlan(definitions: Definition): Step[] {
        return definitions.knowledgeSources.map((ks) => DeleteKSStep.of(ks));
    }
}
